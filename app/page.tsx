...
const fetchFirebaseData = async (): Promise<SensorReading | null> => {
  try {
    ...
    const response = await fetch(`${firebaseConfig.databaseURL}/readings.json?orderBy="$key"&endAt="9999-99-99"&limitToLast=5`, {
...
const fetchAllFirebaseData = async (limit?: number): Promise<SensorReading[]> => {
  try {
    if (!isFirebaseConfigured()) {
    ...
    console.log("[v0] Fetching historical data from Firebase...")
    ...
    while (retryCount < maxRetries) {
      try {
        const queryUrl = limit 
          ? `${firebaseConfig.databaseURL}/readings.json?orderBy="$key"&endAt="9999-99-99"&limitToLast=${limit}`
          : `${firebaseConfig.databaseURL}/readings.json?orderBy="$key"&endAt="9999-99-99"`;
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (testData && dataSource === "firebase") {
          ...
          if (initial24hData.length > 0) {
            setHistoricalData(initial24hData)
            setSensorData(initial24hData.slice(-20))
            const latest = initial24hData[initial24hData.length - 1]
            setCurrentReading(latest)
            
            const dataAge = Math.abs(Date.now() - new Date(latest.timestamp).getTime())
      ...
      <div className="relative z-10">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="xl:col-span-2">
              <div className="w-full overflow-hidden rounded-lg shadow-sm bg-white">
                <TemperatureChart24h data={historicalData} />
              </div>
            </div>
            <div className="w-full overflow-hidden rounded-lg shadow-sm bg-white">
              <HumidityChart24h data={historicalData} />
            </div>
            <div className="w-full overflow-hidden rounded-lg shadow-sm bg-white">
              <LouverStatusChart24h data={historicalData} />
            </div>
          </div>
        </div>
